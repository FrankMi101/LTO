...
{
    [TestClass()]
    public class GeneralItemsAsyncTests
    {
       ...
       // private readonly IAppServicesAsync _action = new AppServiceAsync(new GeneralItemsAsync(DBConnection.DBSource));
             private readonly IAppServicesAsync _action = new AppServiceAsync( new GeneralItemsAsync("Fake"));
        ...
        public async Task GetAppListItems_ReturnSelectItemName(string Operate, object initialValue, object expect)
        {
            //Arrange
            var action = "DDList";
            ...
            Task.Run(async () =>
                       {
                //Act
                var list = await _action.AppOne.CommonList<NameValueList>(action, para);
